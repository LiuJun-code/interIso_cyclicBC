...
void Foam::reconstruction::plicRDF::interpolateNormal()
{
    ...
    exchangeFields_.setUpCommforZone(interfaceCell_,false); // all values (interfaceCell_, centre_, normal_) are known
    ...
    DynamicField<vector > cellCentre(100); ////- Construct empty field with given reserve size.
    ...
    forAll(interfaceLabels_, i)
    {
        ...
        {
            const label gblIdx = stencil[celli][i]; // point neighbors of cell i that are interface cells
            vector n =
            ...
            if (mag(n) != 0)
            {
                ...
                exchangeFields_.centerTransformation(celli, gblIdx, centre);
        ...
        else
        {
            ...
            forAll(stencil[celli],i)
            {
                ...
                vector c = exchangeFields_.getValue(mesh_.C(), mapCC, gblIdx);
                exchangeFields_.centerTransformation(celli, gblIdx, c);
                cellCentre.append
                (
                    c
...
{
    const bool uptodate = alreadyReconstructed(forceUpdate); // forceUpdate default to true
    ...
    if (mesh_.topoChanging()) // no change, skip
    {
    ...
    interfaceCell_ = false; // reasign the field: all values in interfaceCell list is set to false
                            // interfaceCell_(alpha1_.mesh().nCells(), false)-->
    // Sets interfaceCell_ and interfaceNormal
    setInitNormals(interpolateNormal_); // interface center and area norm are initialised by gradient of alpha (false) or old value (true)

    centre_ = dimensionedVector("centre", dimLength, Zero); // initialise interface centre
    normal_ = dimensionedVector("normal", dimArea, Zero);  // initialise area norm

    // nextToInterface is update on setInitNormals
    const boolList& nextToInterface_ = RDF_.nextToInterface(); // if next to interface, size = mesh.size, based on alpha1.oldTime()

    labelHashSet tooCoarse;

    for (int iter=0; iter<iteration_; ++iter) // get from dict entry : fvSolution.alpha.water.iteration, default 5
    {
        forAll(interfaceLabels_, i)
        {
            const label celli = interfaceLabels_[i];
...

        {
            ...
            gradSurf(RDF_); // update interfaceNormal_
