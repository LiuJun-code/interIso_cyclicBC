...
void Foam::isoAdvection::timeIntegratedFlux()
{
    ...
    // Storage for isoFace points. Only used if writeIsoFacesToFile_, NOTE: isoFacePts is an empty list of lists
    DynamicList<List<point>> isoFacePts;
    const DynamicField<label>& interfaceLabels = surf_->interfaceLabels(); //- List of cells with an interface
    ...
    forAll(interfaceLabels, i)
    {
        ...
        if (mag(surf_->normal()[celli]) != 0)
        {
            ...
            nSurfaceCells++;
            surfCells_.append(celli); // used to write out interface cells
    ...
    forAll(bsFaces_, i)
    {
        ...
        if (phib[patchi].size()) // if patchi contains more than zero face
        {
...
//    dVf_.correctBoundaryConditions(); --> no update/evaluate function is applied to fvsPatchField
